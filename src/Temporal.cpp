...
void vk::Temporal::CreateShaderBindingTable()
{
	...
	MissShaderBindingTable->WriteToBuffer(shaderHandleStorage.data() + handle_size_aligned, handle_size_aligned);
...
void vk::Temporal::BuildDescriptors()
{
	...
	for (size_t i = 0; i < (size_t)MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		UpdateDescriptorSet(context, 13, imageInfo, m_descriptorSets[i], VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
	}
